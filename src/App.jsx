      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "32px" }}>
        <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e8e4df", padding: 32 }}>
          <div style={{ marginBottom: 24 }}>
            <label style={{ display: "block", fontWeight: 600, fontSize: 13, color: "#5c5549", marginBottom: 8 }}>Firm Name</label>
            <input
              type="text"
              value={formData.name}
          ...
          <div style={{ marginBottom: 32 }}>
            <label style={{ display: "block", fontWeight: 600, fontSize: 13, color: "#5c5549", marginBottom: 8 }}>JSON Data</label>
            <textarea
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 32 }}>
            {[
              ...
              <div key={img.key}>
                <label style={{ display: "block", fontWeight: 600, fontSize: 13, color: "#5c5549", marginBottom: 8 }}>{img.label}</label>
                <input
                  type="file"
                  accept="image/*"
                ...
                {formData.images[img.key] && (
                  <div style={{ marginTop: 8, fontSize: 12, color: "#5c6d5e" }}>✓ Uploaded</div>
                )}
              </div>
            ))}
          </div>
      ...
      {showDuplicateWarning && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
          <div style={{ background: "#fff", borderRadius: 12, padding: 40, maxWidth: 400, textAlign: "center" }}>
            ...
            <h2 style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 24, fontWeight: 600, margin: "0 0 12px 0", color: "#1a1a1a" }}>Firm Already Exists</h2>
            <p style={{ fontSize: 14, color: "#6b6358", marginBottom: 32 }}>"{existingFirm}" is already in your database. Do you want to replace the existing data?</p>
            <div style={{ display: "flex", gap: 12 }}>
              <button onClick={() => { setShowDuplicateWarning(false); setExistingFirm(null); }} style={{ flex: 1, padding: "12px 20px", background: "#fff", color: "#1a1a1a", border: "1px solid #d6d0c8", borderRadius: 8, fontFamily: "'DM Sans', sans-serif", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>Cancel</button>
              <button onClick={handleReplaceConfirm} style={{ flex: 1, padding: "12px 20px", background: "#c17817", color: "#fff", border: "none", borderRadius: 8, fontFamily: "'DM Sans', sans-serif", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>Replace Data</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showSuccess && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
          <div style={{ background: "#fff", borderRadius: 12, padding: 40, maxWidth: 400, textAlign: "center" }}>
            ...
            <h2 style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 24, fontWeight: 600, margin: "0 0 12px 0", color: "#1a1a1a" }}>Success!</h2>
            <p style={{ fontSize: 14, color: "#6b6358", marginBottom: 32 }}>Competitor data has been imported and saved.</p>
            <div style={{ display: "flex", gap: 12 }}>
              <button onClick={onAddAnother} style={{ flex: 1, padding: "12px 20px", background: "#fff", color: "#5c6d5e", border: "1px solid #5c6d5e", borderRadius: 8, fontFamily: "'DM Sans', sans-serif", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>Add Another</button>
              <button onClick={() => { onCloseSuccess(); onBack(); }} style={{ flex: 1, padding: "12px 20px", background: "#5c6d5e", color: "#fff", border: "none", borderRadius: 8, fontFamily: "'DM Sans', sans-serif", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>Back to Home</button>
            </div>
          </div>
        </div>
      )}
